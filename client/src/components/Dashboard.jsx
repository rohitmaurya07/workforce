...
const StatCard = ({ label, value, icon, user }) => (
  <div className=" primary-border-2 text-accent  rounded-2xl p-5 flex flex-col gap-3">
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-5 md:ml-38 md:ml-58  px-4 py-6 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <p className="text-xs font-medium text-accent uppercase tracking-widest mb-1">Overview</p>
          <h1 className="text-2xl font-semibold ">Dashboard</h1>
          <p className="text-slate-500 text-sm mt-1">
            Welcome back, <span className="text-accent">{user?.name ?? 'there'}</span>. Here's what's happening today.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          {/* Completion ring */}
          <div  className=" bg-primary text-white  rounded-2xl p-6 flex flex-col items-center justify-center gap-3">
            <p className="text-xs  font-medium uppercase tracking-widest">Task Completion</p>
          ...
          {/* Status breakdown */}
          <div  className="lg:col-span-2 bg-primary text-white rounded-2xl p-6 flex flex-col gap-4">
            <p className="text-xs  font-medium uppercase tracking-widest">Status Breakdown</p>
        ...
        {isAdmin && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="primary-border-2 text-accent border-slate-800 rounded-2xl p-6 flex items-center gap-5">
              ...
              <div>
                <p className="text-2xl font-semibold ">{d?.totalProjects ?? '—'}</p>
                <p className="text-xs mt-0.5">Active Projects</p>
              </div>
            </div>
            <div className=" primary-border-2 text-accent border-slate-800 rounded-2xl p-6 flex items-center gap-5">
              ...
              <div className=''>
                <p className="text-2xl font-semibold ">{d?.totalUsers ?? '—'}</p>
                <p className="text-xs  mt-0.5">Team Members</p>
              </div>
            </div>
          </div>
        )}
